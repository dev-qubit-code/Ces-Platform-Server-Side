...
public class StudentInfoConfiguration : IEntityTypeConfiguration<StudentInfo>
{
    public void Configure(EntityTypeBuilder<StudentInfo> builder)
    {
        ...
        builder.HasMany(si => si.Skills).WithMany(s => s.StudentInfos).UsingEntity<StudentInfoSkill>();

        builder.HasMany(si => si.Sources).WithOne(s => s.StudentInfo).OnDelete(DeleteBehavior.Cascade);
